...
class CreditIndex extends Component
{
    ...
    public function render(CreditAlertService $alerts)
    {
        $today = now()->startOfDay();
        $soon = now()->addDays($alerts->daysAhead())->endOfDay();
        $credits = Credit::query()
            ...
            ->when($this->dueFilter === 'today', fn ($query) => $query->where('balance', '>', 0)->whereDate('due_date', $today)->where('status', '!=', CreditStatus::Cancelled))
            ->when($this->dueFilter === 'soon', fn ($query) => $query->where('balance', '>', 0)->whereDate('due_date', '>', $today)->whereDate('due_date', '<=', $soon)->where('status', '!=', CreditStatus::Cancelled))
            ->latest()

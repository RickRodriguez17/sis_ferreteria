...
class ProductForm extends Component
{
    ...
    {
        $this->validate(['name' => ['required', 'string', 'max:255'], 'code' => ['nullable', 'string', 'max:100', Rule::unique('products', 'code')->ignore($this->product?->id)], 'barcode' => ['nullable', 'string', Rule::unique('products', 'barcode')->ignore($this->product?->id)], 'categoryId' => ['required', 'exists:categories,id'], 'brandId' => ['required', 'exists:brands,id'], 'unitId' => ['required', 'exists:units,id'], 'minStock' => ['nullable', 'numeric', 'decimal:0,4', 'min:0'], 'cost' => ['nullable', 'numeric', 'decimal:0,4', 'min:0'], 'presentations' => ['required', 'array', 'min:1'], 'presentations.*.name' => ['required', 'string'], 'presentations.*.equivalence' => ['required', 'numeric', 'gt:0'], 'presentations.*.price_without_invoice' => ['required', 'numeric', 'min:0'], 'presentations.*.price_with_invoice' => ['required', 'numeric', 'min:0'], 'images.*' => ['image', 'max:5120']]);

<div>
    ...
    <div class="rounded-xl bg-white p-5 shadow-sm">
        <x-table-toolbar wire:model.live.debounce.300ms="search">
            <select wire:model.live="status" class="rounded-lg border-slate-300 text-sm"><option value="">Estados</option>@foreach($statuses as $item)<option value="{{ $item->value }}">{{ $item->label() }}</option>@endforeach</select>
        ...
        <div class="mt-5 overflow-x-auto"><table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($quotations as $quotation)
                    <tr><td class="px-3 py-3"><div class="font-medium">{{ $quotation->code }}</div><div class="text-xs text-slate-500">{{ $quotation->created_at?->format('d/m/Y') }}</div></td><td class="px-3 py-3">{{ $quotation->customer?->name ?: 'Cliente ocasional' }}</td><td class="px-3 py-3">{{ money((float) $quotation->total) }}</td><td class="px-3 py-3">{{ $quotation->status->label() }}</td><td class="px-3 py-3 text-right"><a href="{{ route('quotations.show', $quotation) }}" class="text-indigo-600">Ver</a>@can('update', $quotation) @if($quotation->status->value === 'open')<a href="{{ route('quotations.edit', $quotation) }}" class="ml-3 text-slate-600">Editar</a>@endif @endcan @can('create', \App\Models\Quotation::class)<button wire:click="duplicate({{ $quotation->id }})" class="ml-3 text-slate-600">Duplicar</button>@endcan</td></tr>
                @empty

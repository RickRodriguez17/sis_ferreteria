<div>
    ...
    <div class="rounded-xl bg-white p-5 shadow-sm">
        ...
        @if($returnItems !== [])
            <div class="mt-6 overflow-x-auto"><table class="w-full text-left text-sm"><thead class="border-b text-xs uppercase text-slate-500"><tr><th class="px-3 py-3">Producto</th><th class="px-3 py-3">Disponible</th><th class="px-3 py-3">Precio/costo</th><th class="px-3 py-3">Cantidad a devolver</th></tr></thead><tbody class="divide-y divide-slate-100">@foreach($returnItems as $index => $item)<tr><td class="px-3 py-3">{{ $item['product'] }}</td><td class="px-3 py-3">{{ qty($item['available']) }}</td><td class="px-3 py-3">{{ money($item['unit_price']) }}</td><td class="px-3 py-3"><input type="number" min="0" max="{{ $item['available'] }}" step="0.0001" wire:model="returnItems.{{ $index }}.quantity" class="w-40 rounded-lg border-slate-300"><x-input-error :messages="$errors->get('returnItems.'.$index.'.quantity')"/></td></tr>@endforeach</tbody></table></div>
        @endif
